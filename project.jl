...
function simulate_ruin_probability(years, num_sims)
    ...
    # Adjusted for weekly simulation (52 weeks per year)
    Pi = BEL_0 + (r_rate - i_rate) * SCR_0 / 52
    ...
    # For plotting paths
    paths = zeros(total_weeks + 1, min(num_sims, 100))
    paths[1, :] .= C_0
    ...
    for k in 1:num_sims
        ...
        for t in 1:total_weeks
            ...
            if t % 52 == 0
                y = div(t, 52)
                if y in keys(capitals_snapshots)
                    capitals_snapshots[y][k] = C
                end
            end
            ...
            if !is_ruined && C < 0
                ...
                current_year = div(t - 1, 52) + 1
                for future_t in current_year:years
...
begin
    ...
    function plot_paths(limit_t)
        weeks_limit = limit_t * 52
        time_axis = (0:weeks_limit) ./ 52
        p = plot(time_axis, paths[1:weeks_limit+1, :], 
            title="Paths (0-$limit_t yrs)", 
            xlabel="Time (years)", ylabel="Capital", 
